...
export function Sidebar({ sections, collapsed, mobileOpen, onToggleCollapse, onToggleMobile, onNavigate }: SidebarProps) {
  return (
    <>
      ...
      <div
        className={[
          "fixed inset-y-0 left-0 z-30 flex h-full flex-col border-r border-white/50 bg-ink-900/95 px-4 py-5 text-white shadow-panel backdrop-blur-xl transition-transform duration-300 lg:translate-x-0",
          collapsed ? "w-[104px]" : "w-[240px]",
      ...
      >
        <div className="mb-8 flex items-center justify-between gap-3 px-2">
          ...
          <button
            ...
            className="hidden rounded-full border border-white/10 bg-white/5 p-2 text-white/80 transition hover:bg-white/10 lg:inline-flex"
